...
VALUES
('ABC Electronics', 'abc@gmail.com', '9876543210', 'Kolkata');
...
(product_name, category_id, supplier_id,
 price, stock_quantity, discount,
 product_code, status, created_date)
VALUES
...
SELECT
    ...
    CASE
        ...
        WHEN discount <= 20 THEN 'Good Discount'
        ELSE 'Heavy Discount'
...
SELECT
    ...
    p.price,
    p.discount,
    p.price - (p.price * p.discount / 100) AS selling_price,
    CASE
        WHEN p.discount >= 20 THEN 'Best Offer'
        WHEN p.discount >= 10 THEN 'Good Offer'
        ELSE 'Regular Offer'
    END AS offer_status
FROM products p;
...
SELECT
    ...
    order_status,
    CASE
        WHEN order_status = 'PLACED' THEN 'Order Received'
        WHEN order_status = 'PROCESSING' THEN 'Being Prepared'
...
SELECT
    c.customer_name,
    COALESCE(SUM(oi.quantity * oi.unit_price), 0) AS total_purchase
FROM customers c
LEFT JOIN orders o
    ON c.customer_id = o.customer_id
LEFT JOIN order_items oi
...
(
    SELECT price
    FROM products
    WHERE product_name = 'Dell Inspiron'
);
...
-- Get the generated ID for the most recent insert in the session
...
(product_name, category_id, supplier_id,
 ...
 product_code, status, created_date)
VALUES
('Gaming Headset', 4, 1, 5000, 30, 10, 'GAME001', 'ACTIVE', CURDATE());
...
SELECT
    ...
    p.price,
    p.discount,
    p.price - (p.price * p.discount / 100) AS final_selling_price,
    p.stock_quantity,
    CASE
